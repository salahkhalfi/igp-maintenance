...
app.get('/', async (c) => {
    try {
        ...
        const { results: notes } = await c.env.DB.prepare('SELECT * FROM planner_notes ORDER BY priority DESC, created_at DESC').all();
...
app.post('/notes', async (c) => {
    ...
    try {
        const result = await c.env.DB.prepare(
            'INSERT INTO planner_notes (text, time, done, priority, notified) VALUES (?, ?, ?, ?, ?)'
        ).bind(text, time, done ? 1 : 0, priority, notified ? 1 : 0).run();
